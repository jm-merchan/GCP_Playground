...
provider "boundary" {
  addr             = "https://controller-europe-southwest1-78ui.josemerchan-8d4c7e.gcp.sbx.hashicorpdemo.com"
  tls_insecure     = true
  recovery_kms_hcl = <<EOT
    kms "gcpckms" {
    purpose     = "recovery"
    key_ring    = "kms-boundary-keyring-78ui"
    crypto_key  = "kms-boundary-key-recovery-78ui"
