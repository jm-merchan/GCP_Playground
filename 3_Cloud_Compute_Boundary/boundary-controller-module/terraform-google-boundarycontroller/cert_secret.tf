...
resource "acme_certificate" "certificate" {
  ...
  common_name     = "${var.cluster-name}-${var.region}-${random_string.boundary.result}.${local.domain}"
...
resource "tls_self_signed_cert" "ca" {
  ...
  subject {
    common_name = "boundary.server.com"
  }
  ...
  #   command = "echo '${tls_self_signed_cert.ca.cert_pem}' > ./boundary-ca.pem"
  # }
}
...
resource "tls_private_key" "server" {
  ...
  # provisioner "local-exec" {
  #   command = "echo '${tls_private_key.server.private_key_pem}' > ./boundary-key.pem"
  # }
}
...
resource "tls_locally_signed_cert" "server" {
  ...
  # provisioner "local-exec" {
  #   command = "echo '${tls_locally_signed_cert.server.cert_pem}' > ./boundary-crt.pem"
  # }
}
...
locals {
  ...
  boundary_cert = try(lookup(acme_certificate.certificate, "certificate_pem"), "")
  boundary_ca   = try(lookup(acme_certificate.certificate, "issuer_pem"), "")
  boundary_key  = try(lookup(acme_certificate.certificate, "private_key_pem"), "")
}

locals {
  tls_data = {
    #boundary_ca   = base64encode(tls_self_signed_cert.ca.cert_pem)
    boundary_ca = base64encode(local.boundary_ca)
    #boundary_cert = base64encode(tls_locally_signed_cert.server.cert_pem)
    boundary_cert = base64encode(local.boundary_cert)
    #boundary_pk   = base64encode(tls_private_key.server.private_key_pem)
    boundary_pk = base64encode(local.boundary_key)
  }
}
...
resource "google_compute_region_ssl_certificate" "main" {
  ...
  name_prefix = "boundary-${random_string.boundary.result}"
