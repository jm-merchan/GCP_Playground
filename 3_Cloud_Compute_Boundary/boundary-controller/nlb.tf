...
resource "google_compute_region_health_check" "lb" {
  name               = "${var.region}-boundary-interal-lb-${random_string.boundary.result}"
  region             = var.region
...
resource "google_compute_region_backend_service" "lb-ext-api" {
  ...
  name                  = "${var.region}-boundary-external-lb-api-${random_string.boundary.result}"
  region                = var.region
...
resource "google_compute_region_backend_service" "lb-ext-cluster" {
  health_checks         = [google_compute_region_health_check.lb.self_link]
  name                  = "${var.region}-boundary-external-lb-cluster-${random_string.boundary.result}"
  region                = var.region
...
resource "google_compute_region_target_tcp_proxy" "ext-lb-api" {
  name            = "${var.region}-boundary-external-lb-api-${var.region}-${random_string.boundary.result}"
  region          = var.region
...
resource "google_compute_region_target_tcp_proxy" "ext-lb-cluster" {
  name            = "${var.region}-boundary-external-lb-cluster-${var.region}-${random_string.boundary.result}"
  region          = var.region
...
resource "google_compute_forwarding_rule" "ext-lb1" {
  name                  = "${var.region}-boundary-external-api-${random_string.boundary.result}"
  region                = var.region
...
resource "google_compute_forwarding_rule" "ext-lb2" {
  name                  = "${var.region}-boundary-external-cluster-${random_string.boundary.result}"
  region                = var.region
  ip_address            = google_compute_address.public.address
  ip_protocol           = "TCP"
  load_balancing_scheme = "EXTERNAL_MANAGED"
...
resource "google_compute_forwarding_rule" "ext-lb3" {
  name                  = "${var.region}-boundary-external-api443-${random_string.boundary.result}"
  region                = var.region
  ip_address            = google_compute_address.public.address
  ip_protocol           = "TCP"
  load_balancing_scheme = "EXTERNAL_MANAGED"
