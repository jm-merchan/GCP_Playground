...
output "remove_peering_before_destroy" {
  value = "gcloud compute networks peerings delete ${google_service_networking_connection.private_vpc_connection.peering} --network=${google_compute_network.global_vpc.name} --project=${var.project_id}"
}
