...
resource "google_sql_database_instance" "postgres_instance" {
  depends_on          = [time_sleep.wait_20_seconds, google_service_networking_connection.private_vpc_connection]
  name                = "${var.region}-${var.instance_name}-${random_string.boundary.result}"
...
resource "google_sql_database" "postgres_db" {
  name     = "${var.instance_name}${random_string.boundary.result}"
